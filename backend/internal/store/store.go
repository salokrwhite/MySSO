...
type DeveloperAccessStore interface {
	...
	HasManagedUser(ownerUserID, userID string) (bool, error)
	ListManagedUsersPaginated(ownerUserID string, page, pageSize int, appID, emailKeyword string, groupIDs []string, now time.Time) ([]domain.DeveloperManagedUser, int, error)
