...
func New(dependencies *deps.Deps, settingsService *settings.Service) *Service {
	return &RateLimitService{deps: dependencies, settings: settingsService}
}
...
func (s *RateLimitService) CreateSendChallenge(purpose, channel, target, ip, userAgent string) (SendChallengeResult, error) {
	...
	token, err := authutil.GenerateOpaqueToken(24)
	if err != nil {
		return SendChallengeResult{}, err
	}
	now := time.Now().UTC()
	...
	challenge := domain.RequestChallenge{
		...
		Purpose:       purpose,
		Channel:       channel,
		IPHash:        authutil.HashValue(ip),
		UAHash:        authutil.HashValue(userAgent),
		TargetHash:    authutil.HashValue(target),
...
func (s *RateLimitService) RecordFailedAuthAttempt(opts AuthAttemptOptions) error {
	...
	if key := strings.TrimSpace(opts.Identifier); key != "" && policy.AccountLimit > 0 {
		dimensions = append(dimensions, dimension{
			counterKey: fmt.Sprintf("auth:%s:fail:account:%s", opts.Kind, authutil.HashValue(key)),
			lockKey:    fmt.Sprintf("auth:%s:lock:account:%s", opts.Kind, authutil.HashValue(key)),
	...
	if key := strings.TrimSpace(opts.IP); key != "" && policy.IPLimit > 0 {
		dimensions = append(dimensions, dimension{
			counterKey: fmt.Sprintf("auth:%s:fail:ip:%s", opts.Kind, authutil.HashValue(key)),
			lockKey:    fmt.Sprintf("auth:%s:lock:ip:%s", opts.Kind, authutil.HashValue(key)),
	...
	if key := strings.TrimSpace(opts.DeviceID); key != "" && policy.DeviceLimit > 0 {
		dimensions = append(dimensions, dimension{
			counterKey: fmt.Sprintf("auth:%s:fail:device:%s", opts.Kind, authutil.HashValue(key)),
			lockKey:    fmt.Sprintf("auth:%s:lock:device:%s", opts.Kind, authutil.HashValue(key)),
...
func (s *RateLimitService) CheckAndRecordAuthAction(opts AuthAttemptOptions) error {
	...
	if key := strings.TrimSpace(opts.Identifier); key != "" && policy.AccountLimit > 0 {
		dimensions = append(dimensions, dimension{
			counterKey: fmt.Sprintf("auth:%s:action:account:%s", opts.Kind, authutil.HashValue(key)),
			limit:      policy.AccountLimit,
	...
	if key := strings.TrimSpace(opts.IP); key != "" && policy.IPLimit > 0 {
		dimensions = append(dimensions, dimension{
			counterKey: fmt.Sprintf("auth:%s:action:ip:%s", opts.Kind, authutil.HashValue(key)),
			limit:      policy.IPLimit,
	...
	if key := strings.TrimSpace(opts.DeviceID); key != "" && policy.DeviceLimit > 0 {
		dimensions = append(dimensions, dimension{
			counterKey: fmt.Sprintf("auth:%s:action:device:%s", opts.Kind, authutil.HashValue(key)),
			limit:      policy.DeviceLimit,
...
func (s *RateLimitService) authAttemptCounterKeys(opts AuthAttemptOptions) []string {
	...
	if key := strings.TrimSpace(opts.Identifier); key != "" {
		keys = append(keys, fmt.Sprintf("auth:%s:fail:account:%s", opts.Kind, authutil.HashValue(key)))
	}
	if key := strings.TrimSpace(opts.IP); key != "" {
		keys = append(keys, fmt.Sprintf("auth:%s:fail:ip:%s", opts.Kind, authutil.HashValue(key)))
	}
	if key := strings.TrimSpace(opts.DeviceID); key != "" {
		keys = append(keys, fmt.Sprintf("auth:%s:fail:device:%s", opts.Kind, authutil.HashValue(key)))
	}
	return keys
}
...
func (s *RateLimitService) authAttemptLockKeys(opts AuthAttemptOptions) []string {
	keys := make([]string, 0, 3)
	if key := strings.TrimSpace(opts.Identifier); key != "" {
		keys = append(keys, fmt.Sprintf("auth:%s:lock:account:%s", opts.Kind, authutil.HashValue(key)))
	}
	if key := strings.TrimSpace(opts.IP); key != "" {
		keys = append(keys, fmt.Sprintf("auth:%s:lock:ip:%s", opts.Kind, authutil.HashValue(key)))
	}
	if key := strings.TrimSpace(opts.DeviceID); key != "" {
		keys = append(keys, fmt.Sprintf("auth:%s:lock:device:%s", opts.Kind, authutil.HashValue(key)))
	}
	return keys
}

func authAttemptPolicyForKind(kind string, settings settings.SystemSettings) authAttemptPolicy {
...
func (s *RateLimitService) shouldRequireChallenge(settings settings.SystemSettings, channel, ip string) (bool, error) {
	if !settings.SendChallengeEnabled {
		return false, nil
	}
...
func (s *RateLimitService) shouldRequireCaptcha(settings settings.SystemSettings, channel, ip string) (bool, error) {
...
func (s *RateLimitService) validateChallenge(settings settings.SystemSettings, opts PublicSendOptions, channel, purpose, target, ip, userAgent string) (domain.RequestChallenge, error) {
	...
	if challenge.IPHash != authutil.HashValue(ip) || challenge.UAHash != authutil.HashValue(userAgent) {
		return domain.RequestChallenge{}, &SecurityFlowError{Code: "challenge_required"}
	}
	if challenge.TargetHash != "" && challenge.TargetHash != authutil.HashValue(target) {
		return domain.RequestChallenge{}, &SecurityFlowError{Code: "challenge_required"}
	}
...
func (s *RateLimitService) checkTargetCooldown(settings settings.SystemSettings, channel, target string, now time.Time) error {
	var cooldown int
	var key string
	switch channel {
	case "email":
		cooldown = settings.EmailTargetCooldownSeconds
		key = fmt.Sprintf("email:target:cooldown:%s", authutil.HashValue(target))
	case "sms":
		cooldown = settings.SMSTargetCooldownSeconds
		key = fmt.Sprintf("sms:target:cooldown:%s", authutil.HashValue(target))
	default:
		return nil
	}
	if cooldown <= 0 {
...
func (s *RateLimitService) checkSourceLimits(settings settings.SystemSettings, channel, ip, target string, now time.Time) error {
	...
	if uniqueLimit > 0 {
		uniqueKey := fmt.Sprintf("%s:ip:unique_target:hour:%s:%s", channel, ip, authutil.HashValue(target))
...
func (s *RateLimitService) checkGlobalLimits(settings settings.SystemSettings, channel string, now time.Time) error {
...
func (s *RateLimitService) recordSendCounters(settings settings.SystemSettings, channel, target, ip string, now time.Time) error {
	...
	targetCooldownKey := fmt.Sprintf("%s:target:cooldown:%s", channel, authutil.HashValue(target))
...
func (s *RateLimitService) ensureCircuitClosed(settings settings.SystemSettings, channel string, now time.Time) error {
...
func (s *RateLimitService) appendRateLimitEvent(channel, purpose, target, ip, userAgent, result, rule string) error {
	return s.deps.Store.AppendRateLimitEvent(domain.RateLimitEvent{
		...
		UserAgentHash: authutil.HashValue(userAgent),
		Result:        result,
		MatchedRule:   rule,
